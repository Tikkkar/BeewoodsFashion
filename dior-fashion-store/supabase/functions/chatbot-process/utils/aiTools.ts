// ============================================
// utils/aiTools.ts - UPGRADED with Strict Validation & Sequential Exploitation
// ============================================
...
- **VÍ DỤ THỰC TẾ TỪ LỖI:**
    ...
    - **✅ HÀNH ĐỘNG ĐÚNG:**
      {
        "response": "Dạ em đã thêm Set vest vào giỏ hàng của chị rồi ạ. Chị muốn xem thêm mẫu khác hay đặt hàng luôn để em hỗ trợ ạ? 💕",
        "type": "none", "product_ids": [],
        "function_calls": [
          {"name": "add_to_cart", "args": {"product_id": "id_cua_set_vest", "quantity": 1}}
        ]
      }
...
- Nếu data không khớp pattern → HỎI LẠI уточ ("Chị xác nhận địa chỉ là [trích] phải không ạ?")
...
QUY TẮC TRÍCH XUẤT ĐỊA CHỈ:

1. ADDRESS_LINE (BẮT BUỘC):
   ✅ Phải có SỐ NHÀ + TÊN ĐƯỜNG
   ✅ VD hợp lệ:
      • "123 Nguyễn Trãi"
      • "45A Lê Lợi"
      • "78/5 Trần Hưng Đạo"
   ...
   ❌ KHÔNG hợp lệ:
      • "Nguyễn Trãi" (thiếu số nhà)
      • "123" (chỉ có số)
      • "Hà Nội" (chỉ có thành phố)
      • "Quận Hoàn Kiếm" (không phải địa chỉ nhà)
...
3. DISTRICT (optional):
   • Quận 1, Quận 2, ... (TP.HCM)
   • Ba Đình, Hoàn Kiếm, Đống Đa, ... (Hà Nội)
   • Huyện + tên
...
5. PHONE:
   • CHỈ lấy SĐT trong địa chỉ giao hàng (nếu có)
...
🎯 ĐẶC BIỆT: Khi khách nói "gửi về"
→ Tự động thêm sản phẩm BOT vừa giới thiệu vào giỏ
→ Sau đó hỏi thông tin còn thiếu
...
VÍ DỤ:
Khách: "Em lấy áo vest này size M, 2 cái"
→ Gọi: add_to_cart({ "product_id": "uuid-from-context", "size": "M", "quantity": 2 })

Khách: "Gửi về" (Bot vừa gửi product ID: abc-123)
→ Gọi: add_to_cart({ "product_id": "abc-123", "size": "M", "quantity": 1 })
...
VÍ DỤ:
Bot: "Dạ chị vẫn giao về 123 Nguyễn Trãi, Hà Nội phải không ạ?"
Khách: "Đúng rồi"
...
===== FORMAT RESPONSE VỚI FUNCTION =====

Khi cần gọi function, thêm vào JSON response:
  ...
  "tools": [
    {
      "name": "save_customer_info",
      "description":
        "Lưu thông tin cơ bản của khách hàng (tên, SĐT, sở thích). CHỈ gọi khi khách TỰ GIỚI THIỆU, không phải khi cung cấp địa chỉ giao hàng.",
      "parameters": {
        "type": "object",
        "properties": {
          "full_name": {
          ...
          "phone": {
            "type": "string",
            "description":
              "Số điện thoại liên hệ (0xxxxxxxxx hoặc +84xxxxxxxxx). CHỈ lấy khi khách giới thiệu bản thân, KHÔNG lấy từ địa chỉ giao hàng.",
            "pattern": "^[0+][\\d]{9,11}$",
          },
          "style_preference": {
            "type": "array",
            "items": { "type": "string" },
            "description":
              "Phong cách yêu thích (VD: ['thanh lịch', 'trẻ trung', 'sang trọng'])",
          },
          "usual_size": {
            "type": "string",
    ...
    },
    {
      "name": "save_address",
      "description":
        "Lưu địa chỉ giao hàng. CHỈ gọi khi khách CUNG CẤP ĐỊA CHỈ ĐẦY ĐỦ bao gồm: Số nhà + Tên đường + Thành phố. TUYỆT ĐỐI KHÔNG gọi nếu thiếu số nhà hoặc tên đường.",
      "parameters": {
        "type": "object",
        "properties": {
          "address_line": {
            "type": "string",
            "description":
              "Số nhà + Tên đường. VD: '123 Nguyễn Trãi', '45A Lê Lợi'. PHẢI có cả số và tên đường.",
            "pattern": "^\\d+[A-Z]?\\s+.+",
          },
          "ward": {
            "type": "string",
            "description":
              "Phường/Xã (optional). VD: 'Phường Thanh Xuân Trung'",
          },
          "district": {
            "type": "string",
            "description":
              "Quận/Huyện (optional). VD: 'Quận Thanh Xuân', 'Quận 1'",
          },
          "city": {
            "type": "string",
            "description":
              "Thành phố (REQUIRED). VD: 'Hà Nội', 'TP.HCM', 'Đà Nẵng'",
          },
          "phone": {
            "type": "string",
            "description":
              "SĐT người nhận (optional, chỉ khi khác SĐT profile)",
            "pattern": "^[0+][\\d]{9,11}$",
          },
          "full_name": {
            "type": "string",
            "description":
              "Tên người nhận (optional, chỉ khi khác tên profile)",
          },
        },
    ...
    },
    {
      "name": "add_to_cart",
      "description":
        "Thêm sản phẩm vào giỏ hàng. GỌI NGAY khi khách nói 'gửi về', 'gửi cho chị', 'ship về', 'đặt luôn', 'lấy luôn'.",
      "parameters": {
        "type": "object",
        "properties": {
          "product_id": {
    ...
    },
    {
      "name": "confirm_and_create_order",
      "description":
        "Xác nhận và tạo đơn hàng. CHỈ gọi khi: (1) Có sản phẩm, (2) Có địa chỉ, (3) Khách xác nhận HOẶC đã đủ thông tin giao hàng sau khi kích hoạt flow mua hàng ('gửi về').",
      "parameters": {
        "type": "object",
        "properties": {
          "confirmed": {
