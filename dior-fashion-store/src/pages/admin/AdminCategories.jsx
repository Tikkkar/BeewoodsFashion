...
import { getAdminCategories, saveCategory, deleteCategory, uploadImage } from '../../lib/api/admin';
import { Edit, Trash2, Plus, Loader2, Image as ImageIcon, Search } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <h2 className="text-2xl font-bold mb-6">{data.id ? 'Edit' : 'Add'} Category</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-sm font-medium">Active (Show in store)</span>
              </label>
            </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Sắp xếp sản phẩm của bạn vào các danh mục</p>
        </div>
        <button 
        ...
        {filteredCategories.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200">
            {filteredCategories.map(cat => (
              <div key={cat.id} className="flex items-center justify-between p-4 hover:bg-gray-50 transition">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      <span className={`inline-block h-2 w-2 rounded-full ${cat.is_active ? 'bg-green-500' : 'bg-red-500'}`}></span>
                      <span className="font-medium text-lg">{cat.name}</span>
                    </div>
                    <span className="text-gray-400 text-sm">/{cat.slug}</span>
                    <span className="text-gray-400 text-sm ml-3">Order: {cat.display_order}</span>
                  </div>
                </div>
